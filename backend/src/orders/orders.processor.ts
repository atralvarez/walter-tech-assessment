...
export class OrdersProcessor {
	...
	async handleOrderReceived(payload: { orderId: string }) {
		...
		if (!order.autoProcess) {
			this.logger.log(
				`[order.received] Order '${orderId}' is manual-only, skipping auto-advance`,
			);
			return;
		}

		this.logger.log(
			`[order.received] Advancing order '${orderId}' to processing`,
		);
		await sleep(500);
	...
	async handleOrderProcessing(payload: { orderId: string }) {
		...
		if (!order.autoProcess) {
			this.logger.log(
				`[order.processing] Order '${orderId}' is manual-only, skipping auto-advance`,
			);
			return;
		}
		...
		try {
			...
			if (updated.status === "delivered") {
				...
				this.logger.warn(
					`Order '${orderId}' could not be delivered, status: ${updated.status}`,
				);
			}
		} catch (err) {
			this.logger.error(
				`Unexpected error completing order '${orderId}': ${err}`,
			);
		}
	}
}
